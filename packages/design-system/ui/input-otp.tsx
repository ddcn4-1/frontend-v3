...
type InputOTPProps = Readonly<React.ComponentProps<typeof OTPInput>> & {
...
type InputOTPSlotProps = Readonly<React.ComponentProps<'div'>> & {
  readonly index: number;
};

function InputOTPSlot({ index, className, ...props }: InputOTPSlotProps) {
  const inputOTPContext = React.useContext(OTPInputContext);
  const slots = Array.isArray(inputOTPContext?.slots) ? inputOTPContext.slots : [];
  const slot = Number.isInteger(index) ? slots.at(index) : undefined;
